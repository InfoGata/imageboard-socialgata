...
import type {
  ...
  Comment,
  ImageboardId,
  ImageboardOptionsWithHttpRequestFunction,
  Attachment,
} from "imageboard";
...
const SUPPORTED_IMAGEBOARDS: Instance[] = [
  ...
  },
  {
    ...
    bannerUrl:
      "https://anychans.github.io/assets/8e3fc4c9b8a090c7ea71e32701283479.png",
    apiId: "endchan",
  },
  {
    name: "2ch.hk",
            ...
            .map((item: unknown) => {
              ...
              if (typeof item === "object" && item !== null) {
                const obj = item as {
                  ...
                  content?: string;
                  url?: string;
                  meta?: { boardId?: string; threadId?: number; postId?: number };
                };
                if (obj.type === "text") {
                  return obj.content || "";
                }
                ...
                if (obj.type === "post-link") {
                  const commentId = (obj.meta as { commentId?: number })
                    ?.commentId;
...
/**
 * Get Imageboard instance with HTTP request function
 */
const getImageboard = (instanceId: string): Imageboard => {
  ...
  return Imageboard(instanceId as ImageboardId, options);
};
...
const getFeed = async (request?: GetFeedRequest): Promise<GetFeedResponse> => {
  ...
  try {
    ...
    if (!boardId) {
      const communitiesResponse = await getCommunities({ instanceId });
    ...
    const imageboard = getImageboard(instanceId);
...
application.onGetPlatformType = async () => "imageboard";
...
application.onUiMessage = async (message: UiMessageType) => {
  switch (message.type) {
    case "check-settings":
      ...
      break;
    case "save-settings":
      localStorage.setItem(CORS_PROXY_KEY, message.corsProxy);
